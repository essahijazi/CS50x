// Project URL: https://cs50.harvard.edu/x/2024/psets/4/filter/more/
...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        ...
        {
            // Get the average of the pixel's RGB values - this is the grayscale value of the pixel
            int avg = round(((double)image[i][j].rgbtRed + (double)image[i][j].rgbtGreen + (double)image[i][j].rgbtBlue) / 3);

            // Set the grayscale value to the RGB values of the pixel
